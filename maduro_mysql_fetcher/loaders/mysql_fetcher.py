...
# ─────────────────────────────────────────────
def get_sku_list():
    rows = mysql_query("listing_management", """
        SELECT DISTINCT
            ep.sku,
            ep.parent_sku,
            ep.mapped_sku,
        ...
        WHERE ep.which_channel = 1
          AND ep.fulfilment = 'FBM'
          AND ep.sku IS NOT NULL
    """)
    print(f"  Found {len(rows)} FBM SKUs")
...
def fetch_orders(start, end):
    return mysql_query("order_management", """
        ...
        FROM `order` o
        JOIN order_item_info oii ON o.order = oii.order_id
        WHERE o.order_date BETWEEN %s AND %s
          AND o.order_status IN ('completed', 'refunded')
          AND o.order_sub_source = 6,8
    """, (start, end))
...
def fetch_amz_fees(start, end):
    return mysql_query("accounts_management", """
        SELECT
            ...
            SUM(LabmanLabelPurchase_fee)    AS labman_purchase,
            SUM(LabmanLabelReturn_fee)      AS labman_return,
            SUM(LabmanLabelChargeBack_fee)  AS labman_chargeback
        FROM amz_transaction
...
# ─────────────────────────────────────────────
def calc_window(orders, fees_map, ppc_map, refund_map, replacement_map, postage_map):
    sku_data = {}
...
def fetch_and_load(run_id):
    ...
    for window, (start, end) in windows.items():
        ...
        results[window] = calc_window(
            ...
            refund_map, replacement_map, postage_map
        )
    ...
    for sku in all_skus:
        ...
        try:
            ...
            """, (
                run_id, sku, sku, asin, "Unassigned", "Amazon-UK-FBM",
                ...
                d3m.get("margin_pct"),
                (d3m.get("postage", 0) / d3m.get("customer_paid", 1) * 100) if d3m.get("customer_paid") else None,
                (d3m.get("postage", 0) / d3m.get("customer_paid", 1) * 100) if d3m.get("customer_paid") else None,
            ), fetch=False)
            loaded += 1
